...
import com.ss.editor.ui.control.model.node.control.RigidBodyControlModelNode;
...
public class ModelNodeFactory {
    ...
    public static <T, V extends ModelNode<T>> V createFor(@Nullable final T element) {
        ...
        if (element instanceof LayersRoot) {
            ...
            return unsafeCast(
                    new AnimationSpatialTrackModelNode((SpatialTrack) element, ID_GENERATOR.incrementAndGet()));
        }

        if (element instanceof RigidBodyControl) {
            return unsafeCast(
                    new RigidBodyControlModelNode((RigidBodyControl) element, ID_GENERATOR.incrementAndGet()));
        ...
        if (element instanceof LightProbe) {
            ...
            return unsafeCast(
                    new DirectionalLightModelNode((DirectionalLight) element, ID_GENERATOR.incrementAndGet()));
        ...
        if (element instanceof SceneNode) {
            ...
            return unsafeCast(
                    new ParticleEmitterNodeModelNode((ParticleEmitterNode) element, ID_GENERATOR.incrementAndGet()));
        } else if (element instanceof ParticleInfluencers) {
            return unsafeCast(
                    new ParticleInfluencersModelNode((ParticleInfluencers) element, ID_GENERATOR.incrementAndGet()));
        } else if (element instanceof ParticleInfluencer) {
            return unsafeCast(
                    new ParticleInfluencerModelNode((ParticleInfluencer) element, ID_GENERATOR.incrementAndGet()));
            ...
            return unsafeCast(
                    new ParticleGeometryModelNode((ParticleGeometry) element, ID_GENERATOR.incrementAndGet()));
