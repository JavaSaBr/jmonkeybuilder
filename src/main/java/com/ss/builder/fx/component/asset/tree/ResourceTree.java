...
import com.ss.builder.ui.component.asset.tree.context.menu.action.*;
import com.ss.builder.ui.component.asset.tree.resource.*;
...
public class ResourceTree extends TreeView<ResourceElement> {
    ...
    private void startBackgroundRefresh(@NotNull Path assetFolder) {
        ...
        var rootElement = ResourceElementFactory.createFor(assetFolder);
        var newRoot = new TreeItem<ResourceElement>(rootElement);
        newRoot.setExpanded(true);
        ...
        try {
            ...
            expandedElements.forEach(element -> {
                ...
                var item = UiUtils.findItemForValue(newRoot, element);
                if (item == null) {
                    return;
                }
        ...
        EXECUTOR_MANAGER.addFxTask(() -> {
            ...
            try {
                ...
                selectedElements.stream()
                        .map(resourceElement -> UiUtils.findItemForValue(getRoot(), resourceElement))
    ...
    public void notifyCreated(@NotNull Path file) {
        ...
        var fileElement = ResourceElementFactory.createFor(file);
        ...
        var element = ResourceElementFactory.createFor(folder);
        var folderItem = UiUtils.findItemForValue(getRoot(), element);
        ...
        var newItem = new TreeItem<ResourceElement>(ResourceElementFactory.createFor(file));
    ...
    public void notifyMoved(@NotNull Path prevFile, @NotNull Path newFile) {
        ...
        var prevElement = ResourceElementFactory.createFor(prevFile);
        var prevItem = UiUtils.findItemForValue(getRoot(), prevElement);
        if (prevItem == null) {
            return;
        }

        var newParentElement = ResourceElementFactory.createFor(newFile.getParent());
        var newParentItem = UiUtils.findItemForValue(getRoot(), newParentElement);
    ...
    private void processKey(@NotNull KeyEvent event) {
        ...
        if (controlDown && keyCode == KeyCode.V && UiUtils.hasFileInClipboard() &&
