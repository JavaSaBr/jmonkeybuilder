package com.ss.builder.ui.component.editor.impl.scene;

import static com.ss.editor.editor.part3d.impl.scene.AbstractSceneEditor3dPart.KEY_LOADED_MODEL;
...
import com.ss.builder.ui.component.editor.scripting.EditorScriptingComponent;
import com.ss.builder.ui.component.editor.state.impl.BaseEditorSceneEditorState;
import com.ss.builder.ui.component.painting.PaintingComponent;
import com.ss.builder.ui.component.painting.PaintingComponentContainer;
import com.ss.builder.ui.css.CssClasses;
import com.ss.builder.ui.event.impl.FileChangedEvent;
...
import com.ss.editor.editor.part3d.impl.Stats3dPart;
...
public abstract class AbstractSceneFileEditor<M extends Spatial, MA extends AbstractSceneEditor3dPart, ES extends BaseEditorSceneEditorState> extends
     ...
     */
    @NotNull
    private final ComboBox<EditorTransformSupport.TransformationMode> transformModeComboBox;
    ...
    private void updateMaterials(@NotNull Path file) {
        ...
        NodeUtils.visitGeometry(currentModel, geometry -> {
            ...
            var newMaterial = MaterialUtils.updateMaterialIdNeed(file, material);
    ...
    protected void loadState() {
        ...
        var transformType = EditorTransformSupport.TransformType.valueOf(editorState.getTransformationType());
     ...
     */
    @FxThread
    private void changeTransformMode(@NotNull EditorTransformSupport.TransformationMode transformationMode) {
     ...
     */
    @FxThread
    private void updateTransformTool(@NotNull EditorTransformSupport.TransformType transformType, @NotNull Boolean newValue) {
    ...
    protected void createActions(@NotNull HBox container) {
        ...
        FxControlUtils.onSelectedChange(moveToolButton, selected -> updateTransformTool(EditorTransformSupport.TransformType.MOVE_TOOL, selected));
        FxControlUtils.onSelectedChange(rotationToolButton, selected -> updateTransformTool(EditorTransformSupport.TransformType.ROTATE_TOOL, selected));
        FxControlUtils.onSelectedChange(scaleToolButton, selected -> updateTransformTool(EditorTransformSupport.TransformType.SCALE_TOOL, selected));
    ...
    private void applyMaterial(@NotNull DragEvent dragEvent, @NotNull Path file) {
        ...
        executorManager.addJmeTask(() -> {
            ...
            var linkNode = NodeUtils.findParent(geometry, AssetLinkNode.class::isInstance);
    ...
    private void addNewModel(@NotNull DragEvent dragEvent, @NotNull Path file) {
        ...
        executorManager.addJmeTask(() -> {
            ...
            assetLinkNode.setUserData(AbstractSceneEditor3dPart.KEY_LOADED_MODEL, true);
