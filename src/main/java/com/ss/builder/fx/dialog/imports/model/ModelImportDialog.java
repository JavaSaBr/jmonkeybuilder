package com.ss.builder.fx.dialog.imports.model;

import static com.ss.builder.config.DefaultSettingsProvider.Defaults.PREF_DEFAULT_TANGENT_GENERATION;
import static com.ss.builder.config.DefaultSettingsProvider.Preferences.PREF_TANGENT_GENERATION;
...
public class ModelImportDialog extends GenericFileCreator {
    ...
    private void importModelInBackground() {
        ...
        if (editorConfig.getBoolean(DefaultSettingsProvider.Preferences.PREF_TANGENT_GENERATION, DefaultSettingsProvider.Defaults.PREF_DEFAULT_TANGENT_GENERATION)) {
        ...
        var assetFile = notNull(EditorUtils.getAssetFile(modelFile));
    ...
    private void exportMaterials(
        ...
        for (var geometry : geometries) {
            ...
            var assetFile = notNull(EditorUtils.getAssetFile(resultFile));
            var assetPath = EditorUtils.toAssetPath(assetFile);
            ...
            boolean overwriteTextures)
    {
        return oldPath -> {
            ...
            var assetFile = notNull(EditorUtils.getAssetFile(newTextureFile));
            var assetPath = EditorUtils.toAssetPath(assetFile);
