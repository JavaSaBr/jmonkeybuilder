...
import static com.ss.builder.fx.util.UiUtils.findItem;
...
import com.ss.builder.fx.control.tree.node.HideableNode;
...
public class NodeTreeCell<C extends ChangeConsumer, M extends NodeTree<C>> extends TextFieldTreeCell<TreeNode<?>> {
    ...
    private void startDrag(@NotNull MouseEvent mouseEvent) {
        ...
        final TreeItem<TreeNode<?>> treeItem = UiUtils.findItemForValue(treeView, item);
    ...
    private void dragDropped(@NotNull final DragEvent dragEvent) {
        ...
        if (objectId != null) {
            ...
            final TreeView<TreeNode<?>> treeView = getTreeView();
            final TreeItem<TreeNode<?>> dragTreeItem = UiUtils.findItem(treeView, objectId);
            final TreeNode<?> dragItem = dragTreeItem == null ? null : dragTreeItem.getValue();
            if (dragItem == null || !item.canAccept(dragItem, isCopy)) {
                return;
            }

            final TreeItem<TreeNode<?>> newParentItem = UiUtils.findItemForValue(treeView, item);
